...
export const login = async (email, password) => {
  ...
  try {
    const response = await axios.post(ENDPOINTS.login, user, { headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${BEARER_TOKEN}` } });
    console.log('Login response:', response.data); // Para depuración

    const authResponse = new AuthResponse(response.data.status, response.data.message, new AuthData(response.data.data.id, response.data.data.email, response.data.data.created_at, response.data.data.token));
